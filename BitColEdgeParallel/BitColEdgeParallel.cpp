...
void mksub(graph* g, edge ed, subgraph* sg, unsigned k) {
	unsigned i, j, l, w, x, s, t, id_num, id_bit;
...
#pragma omp threadprivate(new0,old0)
	...
	sg->n[k - 2] = j;
	sg->block_num = j / block_size + (j % block_size != 0);
	memset(sg->nodes[k - 2], 0, sizeof(unsigned) * (sg->block_num));
	/*for (i = 0; i < sg->n[k - 1]; i++) {
		id_num = i / block_size, id_bit = i % block_size;
		sg->nodes[k - 1][id_num] |= (1 << id_bit);
	}*/
	j = sg->n[k - 2] / block_size;
	for (i = 0; i < j; i++) { 
		sg->nodes[k - 2][i] = (1 << block_size) - 1;
	}
	j = sg->n[k - 2] % block_size;

	if (j > 0)
		sg->nodes[k - 2][sg->block_num - 1] = (1 << j) - 1;
	j = sg->n[k - 2];
	...
	//color ordering
	unsigned* C = (unsigned*)calloc(sg->n[k - 2], sizeof(unsigned));
	int* color = (int*)malloc(sg->n[k - 2] * sizeof(int));
	unsigned* Index = (unsigned*)malloc(sg->n[k - 2] * sizeof(unsigned));
	iddegree* ig;
	ig = (iddegree*)malloc(sg->n[k - 2] * sizeof(iddegree));
	...
	sg->color = (unsigned*)malloc(sg->n[k - 2] * sizeof(unsigned));
	memset(sg->adj, 0, sizeof(unsigned) * sg->n[k - 2] * sg->block_num);
	for (unsigned i = 0; i < sg->n[k - 2]; i++)
	{
